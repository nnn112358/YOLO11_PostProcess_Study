...
# 設定パラメータ
MODEL_PATH = 'yolo11n_cut_backbone.onnx'            # YOLOv8 ONNXモデルのパス
IMAGE_PATH = 'test.jpg'                 # 検出対象の画像パス
CONFIDENCE_THRESHOLD = 0.4            # 確信度の閾値
NMS_THRESHOLD = 0.4                   # 非最大値抑制の閾値
...
def postprocess(outputs, original_shape, input_size, confidence_threshold, nms_threshold, reg_max=16):
    ...
    for head in heads:
        ...
        if bbox_channels != num_bbox_params * reg_max:
            raise ValueError(f"bbox_channels ({bbox_channels}) が4*reg_max ({4 * reg_max})と一致しません")
...
def main():
    ...
    cv2.imwrite('./test_m5_axera_onnx_result.jpg', cv2.cvtColor(original_image, cv2.COLOR_RGB2BGR))
